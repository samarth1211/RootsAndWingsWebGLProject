...
function samMain() {
  ...
  console.log(
    "Canvas Dimensions Canvas Width:" +
      canvas.width +
      " Canvas Height:" +
      canvas.height +
      " \n"
...
function samInit() {
  ...
  gl.shaderSource(
    vertexShaderObject_final_logo_root,
    vertexShaderSourceCode_final_logo_root
  );
  ...
  gl.shaderSource(
    fragmentShaderObject_final_logo_root,
    fragmentShaderSourceCode_final_logo_root
  );
  ...
  gl.attachShader(
    shaderProgramObject_final_logo_root,
    vertexShaderObject_final_logo_root
  );
  gl.attachShader(
    shaderProgramObject_final_logo_root,
  ...
  if (
    !gl.getProgramParameter(shaderProgramObject_final_logo_root, gl.LINK_STATUS)
  ) {
    var error = gl.getProgramParameter(
      shaderProgramObject_final_logo_root,
      gl.LINK_STATUS
    );
    if (error.length > 0) {
  ...
  uniform_ModelMat_final_logo_root = gl.getUniformLocation(
    shaderProgramObject_final_logo_root,
    "u_m_matrix"
  );
  uniform_ViewMat_final_logo_root = gl.getUniformLocation(
    shaderProgramObject_final_logo_root,
    "u_v_matrix"
  );
  uniform_ProjectionMat_final_logo_root = gl.getUniformLocation(
    shaderProgramObject_final_logo_root,
    "u_p_matrix"
  );
...
function samDraw() {
  ...
  if (control_logo) {
    ...
    gl.uniformMatrix4fv(
      uniform_ProjectionMat_final_logo_root,
      false,
      perspectiveProjectionMatrix
    );
...
function tree() {
  ...
  rami(b1, b2, -thr1, thr2, fir, 1, 0.75, 0.75, 3, 2.5, 2.5, 0.3, 4);
  rami(b1, b3, -thr1, thr2, fir, 0.75, 0.5, 0.5, 1, 1.5, 1.5, 0.3, 4);
  rami(b2, b4, -thr1, thr2, fir, 0.75, 0.5, 0.5, 1, 1.5, 1.5, 0.3, 4);
  rami(b3, b5, -thr1, thr2, fir, 0.5, 0.5, 0.5, 1, 1.5, 1.5, 0.3, 4);

  branch(lev, b1, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch(lev, b2, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch(lev, b3, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch(lev, b4, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch(lev, b5, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
...
function branch(it, b, sr, sb, stu, div) {
  ...
  var di = div[it],
    sr1 = scr1 * sr,
    sr2 = 0.6 * sr;
  twig(b, th1, fi1, 0.8 * sr, sr1, sb, sb, stu, di);
  twig(b2, th2, fi2, 0.8 * sr, sr2, sb, sb, stu, di);
  if (di) stu *= 2;
  it--;
  if (it == 0) {
    if (leaf) {
...
function roots() {
  ...
  branch_root(5, b1_root, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch_root(6, b2_root, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch_root(6, b3_root, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  branch_root(5, b4_root, 0.5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);
  //branch_root(2, b5_root, .5, 1.5, 4, [0, 1, 1, 0, 0, 0, 0, 0, 0]);

  var t_root = off_root / 8,
    i = 0,
    r = 5; // ground
  //var gr = [-r,-r, r,-r, -r,r, r,r];
...
function branch_root(it, b, sr, sb, stu, div) {
  ...
  var di = div[it],
    sr1 = scr1_root * sr,
    sr2 = 0.6 * sr;
  twig_root(b, th1_root, fi1_root, 0.8 * sr, sr1, sb, sb, stu, di);
  twig_root(b2, th2_root, fi2_root, 0.8 * sr, sr2, sb, sb, stu, di);
  if (di) stu *= 2;
  it--;
  if (it == 0) {
    if (leaf) {
